...
export const DrawerNavigationString = `import { Box, Drawer, DrawerContent, Flex, HStack, Heading, IconButton, List, ListIcon, ListItem, Text,  useBreakpointValue,  useColorModeValue, useDisclosure  } from '@chakra-ui/react'
    ...
    return (
        <>
            <Flex as="nav" alignItems="center" justifyContent={{ base: 'space-between', lg: 'flex-end' }} h='10vh' p='2.5'>
                <HStack spacing={2} display={{ base: 'flex', lg: 'none' }}>
                    <IconButton {...buttonProps} fontSize="18px" variant='ghost' icon={<Menu size='20' />} aria-label='open menu'/>
                    <Heading as='h1' size="md">Brand</Heading>
                </HStack>
                <HStack spacing="1">
                    <IconButton variant="ghost" isRound={true} size="md" aria-label='earth icon' icon={<EarthIcon size="20" />}/>
                    <IconButton isRound={true} size="md" aria-label='user icon' icon={<User size='20' />}  />
                </HStack>
            </Flex>
            <HStack align="start" spacing={0}>
...
const Aside = ({ onClose, isOpen, ...rest }: AsideProps) => {
    return (
        ...
        >
            <HStack p="2.5" h='10vh' justify="space-between">
                <Heading as='h1' size="md">Brand</Heading>
                <IconButton onClick={onClose}  display={isOpen ? 'flex' : 'none'} fontSize="18px" variant='ghost' icon={<X size="20" />} aria-label='open menu'/>
            </HStack>
            <Box> 
              <List spacing={0} p="0.5">
                {
                  listItems.map((item, index) => (<ListElement key={index} icon={item.icon} text={item.text} />))
                }
              </List>
            </Box>
        </Box>
